...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
    ...
    SET {sep}{sets})";
        ...
        private string GenerateCypher(CypherFormat cypherFormat)
        {
            ...
            try
            {
                sb = GenerateCypher(cypherFormat, sb);
                cypher = sb.ToString();
            ...
            CypherFormat cypherFormat)
        {
            ...
            sb = cypherFormat switch
            {
                ...
                CypherFormat.MultiLine => FormatMultiLine(current, sb, repeat)
                                                           .FormatStatement(current, repeat),
        ...
        private static StringBuilder FormatSingleLine(FluentCypher current, StringBuilder sb)
        {
            ...
            char last2 = sb.Length > 1 ? sb[sb.Length - 2] : '_';
            char last3 = sb.Length > 2 ? sb[sb.Length - 3] : '_';
            bool space = (last1, last2, last3) switch
            {
                (' ', _, _) => false,
                (',', ' ', _) => false,
                //(' ', ',', _) => false,
                //(_, char b, char c) when c != SPACE_CHAR && b != SPACE_CHAR => true,
                _ => true
            };
            if (space)
                sb = sb.Append(SPACE);
        ...
        /// <returns></returns>
        private static StringBuilder FormatMultiLine(FluentCypher current, StringBuilder sb, int repeat)
        {
            if (sb.Length == 0)
                return sb;
        ...
        private static StringBuilder FormatMultiLineDense(FluentCypher current, StringBuilder sb, int repeat)
        {
            ...
            {
                sb = FormatMultiLine(current, sb, repeat);
                return sb;
            }
        ...
        private static int RepeatCount(FluentCypher current)
        {
            ...
            while (previous != null && i < BREAK_LINE_ON)
            {
                ...
                if (prevPhrase == CypherPhrase.None ||
                    prevPhrase == CypherPhrase.Or ||
                    prevPhrase == CypherPhrase.And)
                {
